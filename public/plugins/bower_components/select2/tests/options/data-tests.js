...
test('no nesting', function (assert) {
  var $test = $('<select data-index="index"></select>');
...
test('with nesting', function (assert) {
  var $test = $('<select data-first--second="index"></select>');

  if ($test[0].dataset == null) {
    assert.ok(
      true,
      'We can not run this index with jQuery 1.x if dataset is not implemented'
    );
